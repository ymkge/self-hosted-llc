...
民間サービス（マネーフォワード/freee等）を使わず、AIと政府公式ツールのみを利用して、法定費用最小構成で合同会社（LLC）を設立・運用するためのナレッジベースです。
...
- [ ] 類似商号の調査（[国税庁法人番号公表サイト](https://www.houjin-bangou.nta.go.jp/) 等で重複や類似企業を確認）
- [ ] AIを活用した事業目的（Business Objectives）の策定・精査
- [ ] 法人実印（代表社員之印）の発注（詳細は [Setup Guide](./docs/setup_guide.md) 参照）

### Phase 2: Build (書類作成)
- [ ] AIによる定款ドラフトの生成（`__self.md` として保存）
- [ ] 代表社員就任承諾書、本店所在地決定書等の作成
- [ ] 法人実印の受取と印影データの作成（スキャンしてJPG/PNG保存）
- [ ] 資本金の入金（**定款作成日以降**に実施）と通帳コピー（PDF）の準備

### Phase 3: Deploy (電子申請)
- [ ] PC/スマホの環境疎通確認（マイナポータル等で電子署名が動作するかテスト）
...
- [ ] 登記簿謄本（履歴事項全部証明書）の取得
- [ ] 法人銀行口座の開設
- [ ] 税務署・自治体への設立届出（詳細は [Post-Deployment](./docs/post_deployment.md) 参照）
